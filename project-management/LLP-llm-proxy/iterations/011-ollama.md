...
| Test | Result |
|------|--------|
| `auto` → ollama completion | ✅ `served_by: ollama`, content "PONG", usage `{prompt:30, completion:3}` |
| Streaming (`stream: true`) | ✅ SSE chunks, role delta, content, finish_reason=stop, usage in final chunk |
| Model override `ollama/dolphin3:8b` | ✅ Pinned + served |
...
**Decision:** Wire HttpProvider to local Ollama (container, GPU, dolphin3:8b default), rename openllm → ollama
