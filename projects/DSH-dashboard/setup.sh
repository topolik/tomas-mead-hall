...
printf "  Checking Tailscale..."
ts_state=""
for i in $(seq 1 5); do
  ts_state=$(docker compose exec -T tailscale tailscale status --json 2>/dev/null | jq -r .BackendState) && break
  printf "."
  sleep 1
done
echo ""
...
if [ "$ts_state" = "Running" ]; then
  ...
  ok "Tailscale already authenticated (https://${dns})"
else
  echo ""
